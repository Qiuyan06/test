<template>
	<u-popup v-model="editShow" mode="right" width="90%" :closeable="true" :mask-close-able="true" class="editBox">
		<view class="headTitle">{{type==='add' ? '保养新增' :'保养编辑'}}</view>
		<u-form :model="maintainForm" ref="maintainRef">
			<u-form-item prop="serverTime">
				<view class="editItem">
					<view class="title">保养时间</view>
					<view class="wrap" style="margin:20rpx 0;">
						<view class="wrap_input" @click="timeShow = true">
							<text class="val" :class="maintainForm.serverTime?'selected':''">
								{{maintainForm.serverTime || '请选择时间'}}
							</text>
						</view>
				...
				<view class="editItem">
					...
					<view class="wrap">
						<view class="wrap_input">
							<u-input v-model="maintainForm.amount" placeholder="请输入金额" type="number" />
						</view>
			...
			<u-form-item prop="remark" :border-bottom="false">
				<view class="editItem">
					...
					<view class="wrap remark">
						<view class="wrap_input">
							<u-input type="textarea" v-model="maintainForm.remark" placeholder="请输入信息" />
						</view>
					</view>
				</view>
			</u-form-item>
...
<script>
	...
	export default {
		...
		methods: {
			...
			timeConfirm(data) {
				this.$set(this.maintainForm, 'serverTime', `${data.year}-${data.month}-${data.day}`)
			},
			isCheck() {
				// 校验维护时间
				if (!this.maintainForm.serverTime) {
					uni.showToast({
						title: '请选择时间',
						icon: 'none'
					});
				...
				if (!this.maintainForm.amount) {
					...
					if (!(/^[+]?\d+(.\d{0,2})?$/.test(this.maintainForm.amount))){
						uni.showToast({
							title: '请输入正确的金额',
							icon: 'none'
						});
				...
				if (!this.maintainForm.remark) {
					...
					if (this.maintainForm.remark.length > 300) {
						uni.showToast({
			...
			handleSubmit() {
				if (this.isCheck()) {
					const newAmount = new Decimal(this.maintainForm.amount).mul(100)
					if(this.type === 'add'){
						this.$emit('save', { ...this.maintainForm,
							type: '2',
							amount: newAmount.valueOf()
						})
					}else{
						this.$emit('update',{ ...this.maintainForm,
							id:this.id,
							type: '2',
							amount: newAmount.valueOf()
						})
