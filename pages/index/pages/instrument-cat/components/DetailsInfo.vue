<template>
	<view class="DetailsInfo">
		<view class="content">
			...
			<!-- 	<view class="basic-item">
					<text style="margin-right: 100rpx">当前电流：{{outcurrent}}A</text>
					<text>当前电压：{{outvoltage}}V</text>
				</view> -->
...
<script>
	export default {
		...
		computed: {
			...
			getLastTime() {
				return this.instruData.purchaseTime.slice(0, 10).replace(/(^\d{4}(?=.))(.+)(\d{2}$)/g, ($, $1, $2, $3) => (Number($1) +
					5) + $2 + (Number($3) - 1))
			},
		},
		methods: {
			toInfo() {
				...
				uni.navigateTo({
					url: `/pages/index/pages/instrument-cat/info-page${instruNum}${instruId}${dbName}${jurisdiction}`
				});
			},
