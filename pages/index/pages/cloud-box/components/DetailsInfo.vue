	...
	<view class="DetailsInfo">
		<view class="content">
			<view class="basic">
				<view class="name" @click="toInfo">
					...
					<u-icon name="arrow-right" color="#111737" size="24"
						style="position: relative;top: 2rpx;left: 4rpx;" />
				</view>
			...
			<view class="statistics">
				<view class="statistics-item" v-if="showWarnEntry" @click="toRecord(0)">
					<view class="num">
						<u-badge :count="(recordData && recordData.notAlarmRecord || 0)" :is-center="true"></u-badge>
						<text>{{recordData && recordData.alarmRecord || 0 }}</text>
					</view>
				...
				<view class="statistics-item" @click="toRecord(showWarnEntry?1:0)">
					<view class="num">
						<text>{{recordData && recordData.maintainRecord || 0}}</text>
					</view>
					<text class="label">维护</text>
				</view>
				<view class="statistics-item" @click="toRecord(showWarnEntry?2:1)">
					<view class="num">
						<text>{{recordData && recordData.measurementRecord || 0}}</text>
					</view>
			...
			<!-- <view class="statistics" @click="toRecord">
				<view class="statistics-item">
					<view class="num">
				...
				<view class="statistics-item">
					<view class="num">
						<text>{{recordData ? recordData.maintainRecord || 0 : 0}}</text>
					</view>
					<text class="label">维护</text>
				</view>
				<view class="statistics-item">
					<view class="num">
						<text>{{recordData ? recordData.measurementRecord || 0 : 0}}</text>
					</view>
					<text class="label">计量</text>
				</view>
				<view class="statistics-item">
	...
	export default {
		...
		methods: {
			toInfo() {
				...
				const instruType = this.instruData.instruType
				let equipNum = ''
				if (instruType === 1) {
					equipNum = `&equipNum=${this.instruData&&this.instruData.serializeNum||''}`
				} else {
					equipNum = `&equipNum=${this.instruData&&this.instruData.instruNum||''}`
				}

				uni.navigateTo({
					url: `/pages/index/pages/cloud-box/info-page${instruNum}${instruId}${dbName}${jurisdiction}${equipNum}`
				});
			},
			// 跳转到记录页面
			toRecord(tabsCurrent) {
				...
				// 云盒子配置过报警跳转到新记录页面，否则跳转至旧记录页面
				if(this.isToCloudBoxWarn){
                    const payload = this.showWarnEntry ? this.warnLogger : []
					this.$u.vuex(`s_bus.setList`, payload)
					uni.navigateTo({
						url: `/pages/index/pages/device-cloud-box-record/index${instruNum}${instruId}${dbName}${jurisdiction}${current}&instruType=${this.instruType}`
					})
				}else{
					uni.navigateTo({
				...
				/*//跳转到云盒子报警页面
				this.$u.vuex(`s_bus.setList`, this.warnLogger)
				uni.navigateTo({
					url: `/pages/index/pages/device-cloud-box-record/index${instruNum}${instruId}${dbName}${jurisdiction}${current}&instruType=${this.instruType}`
				})*/
			},
			getLifeCycleTime() {
				...
				this.$u.api.equipmentApi.getLifeCycle()
			}
